...
export default function MedicalLeaves() {
  ...
  const filtered = useMemo(() => {
    return medicalLeaves.filter((leave) => {
      ...
      if (!vcStep) return false;

      if (searchTerm) {
        const q = searchTerm.toLowerCase();
        const match =
          leave.employee?.name?.toLowerCase().includes(q) ||
          leave.employee?.code?.toLowerCase().includes(q) ||
          leave.employeeName?.toLowerCase().includes(q);
        if (!match) return false;
      }

      if (activeTab === 'all') return true;
      if (activeTab === 'pending') return vcStep.status === 'pending';
      if (activeTab === 'approved') return vcStep.status === 'approved';
      if (activeTab === 'rejected') return vcStep.status === 'rejected';
      return true;
    });
  }, [medicalLeaves, searchTerm, activeTab]);

  const stats = useMemo(() => {
    const vcLeaves = medicalLeaves.filter((l) => l.approvalChain?.some((s) => s.role === 'vc'));
    const get = (status) =>
      vcLeaves.filter((l) => l.approvalChain.find((s) => s.role === 'vc')?.status === status)
        .length;
    return {
      total: vcLeaves.length,
      pending: get('pending'),
