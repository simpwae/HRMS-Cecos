...
export default function ExEmployees() {
  const { exEmployees } = useDataStore();
  ...
  }, [exEmployees, searchTerm, departmentFilter, facultyFilter]);
  ...
  const analytics = useMemo(() => {
    ...
    // Average tenure
    const avgTenure = exEmployees.length
      ? (
          exEmployees.reduce((sum, a) => sum + (a.yearsOfService || 0), 0) / exEmployees.length
        ).toFixed(1)
      : 0;

    // Satisfaction scores
    const satisfactionScores = exEmployees.filter((a) => a.exitSurvey?.satisfaction);
    ...
    const recommendCount = exEmployees.filter((a) => a.exitSurvey?.wouldRecommend).length;
    const recommendPct = exEmployees.length
      ? Math.round((recommendCount / exEmployees.length) * 100)
      : 0;
  ...
  const handleExport = () => {
    const exportData = filteredExEmployees.map((a) => ({
      Name: a.name,
      Email: a.email,
    ...
    link.href = url;
    link.download = `ex-employees-${format(new Date(), 'yyyy-MM-dd')}.csv`;
    link.click();
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          {
            label: 'Total Ex-Employees',
            value: exEmployees.length,
            icon: UserGroupIcon,
            color: 'indigo',
          },
          {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card title="Department Distribution" subtitle="Ex-employees by department">
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
      ...
      {/* Ex-Employees List */}
      <Card title={`Ex-Employees Directory (${filteredExEmployees.length})`}>
        {filteredExEmployees.length === 0 ? (
          <EmptyState
            icon={UserGroupIcon}
            title="No Ex-Employees Found"
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-full bg-linear-to-br from-indigo-400 to-purple-400 flex items-center justify-center text-white font-bold">
                      {exEmployee.name?.charAt(0)}
                    </div>
                    <div>
                      <h4 className="font-semibold text-gray-900">{exEmployee.name}</h4>
                      <p className="text-sm text-gray-500">
                        {exEmployee.designation} • {exEmployee.department}
                      </p>
                  ...
                  <div className="flex flex-wrap items-center gap-4">
                    <div className="text-sm text-gray-500">
                      ...
                      {exEmployee.yearsOfService} years
                    </div>
                    <div className="text-sm text-gray-500">
                      Left:{' '}
                      {exEmployee.exitDate
                        ? format(parseISO(exEmployee.exitDate), 'MMM yyyy')
                        : 'N/A'}
                    </div>
                    <Badge variant="default">{exEmployee.exitReason}</Badge>
                    {exEmployee.exitSurvey && (
                      <ClipboardDocumentCheckIcon
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center gap-4 p-4 bg-linear-to-r from-indigo-50 to-purple-50 rounded-xl">
              <div className="w-16 h-16 rounded-full bg-linear-to-br from-indigo-500 to-purple-500 flex items-center justify-center text-white text-2xl font-bold">
                {selectedExEmployee.name?.charAt(0)}
              </div>
              <div>
                <h3 className="text-lg font-semibold text-gray-900">{selectedExEmployee.name}</h3>
                <p className="text-gray-500">{selectedExEmployee.designation}</p>
                <p className="text-sm text-gray-400">{selectedExEmployee.email}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <p className="font-medium">{selectedExEmployee.department}</p>
              </div>
              <div>
                <p className="text-gray-500">Faculty</p>
                <p className="font-medium">{selectedExEmployee.faculty}</p>
              </div>
              <div>
                <p className="text-gray-500">Join Date</p>
                <p className="font-medium">
                  {selectedExEmployee.joinDate
                    ? format(parseISO(selectedExEmployee.joinDate), 'MMM d, yyyy')
                    : 'N/A'}
                </p>
              </div>
              <div>
                ...
                <p className="font-medium">
                  {selectedExEmployee.exitDate
                    ? format(parseISO(selectedExEmployee.exitDate), 'MMM d, yyyy')
                    : 'N/A'}
                </p>
              </div>
              <div>
                <p className="text-gray-500">Years of Service</p>
                <p className="font-medium">{selectedExEmployee.yearsOfService} years</p>
              </div>
              <div>
                <p className="text-gray-500">Exit Reason</p>
                <p className="font-medium">{selectedExEmployee.exitReason}</p>
              </div>
            </div>

            {/* Exit Survey Results */}
            {selectedExEmployee.exitSurvey && (
              <div className="border rounded-xl p-4">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {[
                      { label: 'Satisfaction', value: selectedExEmployee.exitSurvey.satisfaction },
                      { label: 'Management', value: selectedExEmployee.exitSurvey.management },
                      {
                        label: 'Environment',
                        value: selectedExEmployee.exitSurvey.workEnvironment,
                      },
                      { label: 'Growth', value: selectedExEmployee.exitSurvey.growth },
                    ].map((item) => (
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-3 bg-gray-50 rounded-lg">
                      ...
                      <p className="font-medium">
                        {selectedExEmployee.exitSurvey.wouldRecommend ? '✅ Yes' : '❌ No'}
                      </p>
                    </div>
                    <div className="p-3 bg-gray-50 rounded-lg">
                      <p className="text-sm text-gray-500">Would Return</p>
                      <p className="font-medium">
                        {selectedExEmployee.exitSurvey.wouldReturn ? '✅ Yes' : '❌ No'}
                      </p>
                    </div>
                  </div>

                  {/* Feedback */}
                  {selectedExEmployee.exitSurvey.feedback && (
