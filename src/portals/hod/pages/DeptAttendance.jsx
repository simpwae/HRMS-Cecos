...
export default function DeptAttendance() {
  ...
  // Daily attendance with employee info
  ...
  }, [deptEmployees, attendance, selectedDate, searchTerm]);

  // Summary stats
  const summary = useMemo(() => {
    const records = attendance.filter(
      (a) => a.date === selectedDate && deptEmployeeIds.has(a.employeeId),
    );
    return {
  ...
  }, [attendance, selectedDate, deptEmployees, deptEmployeeIds]);
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `${user.department}_attendance_${selectedDate}.csv`;
    a.click();
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Department Attendance</h2>
          <p className="text-gray-500 mt-1">Track attendance for {user.department}</p>
        </div>
                  ...
                  <tr key={emp.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-linear-to-br from-emerald-500 to-green-500 flex items-center justify-center text-white font-bold text-xs">
                          {emp.name.charAt(0)}
                        </div>
                        <div>
